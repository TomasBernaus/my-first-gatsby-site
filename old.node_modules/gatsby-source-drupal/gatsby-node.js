...
async function worker([url, options]) {
  ...
  httpSpan.setTag(SemanticAttributes.HTTP_SCHEME, parsedUrl.protocol.replace(/:$/, ``));
  ...
  if (initialSourcing) {
    ...
    if (apiRequestCount % 50 === 0) {
      globalReporter.verbose(`gatsby-source-drupal has ${queueLength} API requests queued and the current request rate is ${(apiRequestCount / ((Date.now() - start) / 1000)).toFixed(2)} requests / second`);
    }
  }
  ...
  httpSpan.setTag(SemanticAttributes.HTTP_STATUS_CODE, response === null || response === void 0 ? void 0 : response.statusCode);
  httpSpan.setTag(SemanticAttributes.HTTP_METHOD, `GET`);
  httpSpan.setTag(SemanticAttributes.NET_PEER_IP, response === null || response === void 0 ? void 0 : response.ip);
  httpSpan.setTag(SemanticAttributes.HTTP_RESPONSE_CONTENT_LENGTH, response === null || response === void 0 ? void 0 : (_response$rawBody = response.rawBody) === null || _response$rawBody === void 0 ? void 0 : _response$rawBody.length);
  httpSpan.finish();
  return response;
}
...
exports.onPreBootstrap = ({
  ...
  if (typeof (actions === null || actions === void 0 ? void 0 : actions.addRemoteFileAllowedUrl) === `function`) {
...
${JSON.stringify(webhookBody, null, 4)}`);
      ...
      if (action === `delete`) {
        ...
        for (const nodeToDelete of nodesToDelete) {
          const deletedNode = await (0, _utils.handleDeletedNode)({
            actions,
            getNode,
            cache,
            node: nodeToDelete,
      ...
      for (const nodeToUpdate of nodesToUpdate) {
        await (0, _utils.createNodeIfItDoesNotExist)({
          nodeToUpdate,
          actions,
          createNodeId,
          createContentDigest,
      ...
      for (const nodeToUpdate of nodesToUpdate) {
        await (0, _utils.handleWebhookUpdate)({
          nodeToUpdate,
          actions,
          cache,
          createNodeId,
  ...
  if (fastBuilds) {
    ...
    const lastFetched = (_store$getState$statu = store.getState().status.plugins) === null || _store$getState$statu === void 0 ? void 0 : (_store$getState$statu2 = _store$getState$statu[`gatsby-source-drupal`]) === null || _store$getState$statu2 === void 0 ? void 0 : _store$getState$statu2.lastFetched;
    ...
    if (!lastFetched) {
      ...
      try {
        ...
        const res = await requestQueue.push([urlJoin(baseUrl, `gatsby-fastbuilds/sync/`, Math.floor(lastFetched).toString()), {
        ...
        if (res.body.status === -1) {
          ...
          createNodesSpan.setTag(`sourceNodes.createNodes.count`, (_res$body$entities = res.body.entities) === null || _res$body$entities === void 0 ? void 0 : _res$body$entities.length);
          ...
          for (const nodeSyncData of nodesToSync) {
            if (nodeSyncData.action === `delete`) {
              await (0, _utils.handleDeletedNode)({
                actions,
                getNode,
                cache,
                node: nodeSyncData,
  ...
  const drupalFetchActivity = reporter.activityTimer(`Fetch all data from Drupal`, {
    parentSpan
  });
  ...
  try {
    ...
    allData = await Promise.all(_.map(res.body.links, async (url, type) => {
      ...
      const getNext = async (url, currentLanguage, filterByLanguages, renamedEnabledLanguages) => {
        var _d$body$meta, _d$body$links;
        ...
        if (d.body.data && currentLanguage && filterByLanguages) {
          const languageCodeForFilter = renamedEnabledLanguages && renamedEnabledLanguages.find(language => language.as === currentLanguage) ? renamedEnabledLanguages.find(language => language.as === currentLanguage).langCode : currentLanguage;
          d.body.data = d.body.data.filter(n => n.attributes.langcode === languageCodeForFilter);
        }
        if (d.body.data) {
          // @ts-ignore
        ...
        if ((_d$body$meta = d.body.meta) !== null && _d$body$meta !== void 0 && _d$body$meta.count) {
          var _d$body$links$next;
          const typeLangKey = type + currentLanguage;
          // If we hadn't added urls yet
          if ((_d$body$links$next = d.body.links.next) !== null && _d$body$links$next !== void 0 && _d$body$links$next.href && !typeRequestsQueued.has(typeLangKey)) {
            ...
            const pageSize = Number(new URL(d.body.links.next.href).searchParams.get(`page[limit]`));
            const requestsCount = Math.floor(d.body.meta.count / pageSize);
            reporter.verbose(`queueing ${requestsCount} API requests for type ${type} which has ${d.body.meta.count} entities.`);
            const newUrl = new URL(d.body.links.next.href);
            await Promise.all(_.range(requestsCount).map(pageOffset => {
              // We're starting 1 ahead.
              pageOffset += 1;
              // Construct URL with new pageOffset.
              newUrl.searchParams.set(`page[offset]`, String(pageOffset * pageSize));
              return getNext(newUrl.toString(), currentLanguage, filterByLanguages, renamedEnabledLanguages);
            }));
          }
        } else if ((_d$body$links = d.body.links) !== null && _d$body$links !== void 0 && _d$body$links.next) {
          await getNext(d.body.links.next, currentLanguage, filterByLanguages, renamedEnabledLanguages);
        }
      };
      if (isTranslatable === false) {
        ...
        for (let i = 0; i < languageConfig.enabledLanguages.length; i++) {
          ...
          const renamedEnabledLanguages = getOptions().languageConfig.renamedEnabledLanguages;
          const filterByLanguages = getOptions().languageConfig.filterByLanguages;
  ...
  const nodes = new Map();
  const fileNodesExtendedData = (0, _utils.getExtendedFileNodeData)(allData);
  ...
  for (const contentType of allData) {
    ...
    await asyncPool(concurrentFileRequests, contentType.data, async datum => {
      ...
      const node = await (0, _normalize.nodeFromData)(datum, createNodeId, entityReferenceRevisions, pluginOptions, fileNodesExtendedData, reporter);
      (0, _utils.drupalCreateNodeManifest)({
        attributes: datum === null || datum === void 0 ? void 0 : datum.attributes,
        gatsbyNode: node,
        unstable_createNodeManifest
      });
  ...
  if (skipFileDownloads) {
    ...
    const fileNodes = [...nodes.values()].filter(node => (0, _normalize.isFileNode)(node, pluginOptions.typePrefix));
    if (fileNodes.length) {
...
exports.onCreateDevServer = ({
  ...
  app.use(`/___updatePreview/`, bodyParser.text({
    ...
    if (!_.isEmpty(req.body)) {
      ...
      return await (0, _utils.handleWebhookUpdate)({
        nodeToUpdate,
        actions,
        cache,
        createNodeId,
...
exports.pluginOptionsSchema = ({
  ...
  proxyUrl: Joi.string().description(`The CDN URL equivalent to your baseUrl`),
  ...
  typePrefix: Joi.string().description(`Prefix for Drupal node types`).default(``),
  ...
  languageConfig: Joi.object({
    ...
    enabledLanguages: Joi.array().items(Joi.string(), Joi.object({
  ...
  placeholderStyleName: Joi.string().description(`The machine name of the Gatsby Image CDN placeholder style in Drupal. The default is "placeholder".`)
});
...
exports.createSchemaCustomization = ({
  ...
  if (pluginOptions.skipFileDownloads && pluginOptions.imageCDN) {
    ...
    addRemoteFilePolyfillInterface(schema.buildObjectType({
      name: (0, _normalize.generateTypeName)(`file--file`, pluginOptions.typePrefix),
      fields: {},
      ...
      name: (0, _normalize.generateTypeName)(`files`, pluginOptions.typePrefix),
      fields: {},
      interfaces: [`Node`, `RemoteFile`]
    }), {
      schema,
    ...
    reporter.info(`[gatsby-source-drupal] Enable the skipFileDownloads option to use Gatsby's Image CDN. See the docs for more info:\n${imageCdnDocs}\n`);
  }
};